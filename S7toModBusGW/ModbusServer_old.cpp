	...
	for (vector<DataItem *>::iterator itr = PLC->ListOfDataItems.begin() ; itr != PLC->ListOfDataItems.end(); ++itr)
	{
		switch((*itr)->DetermineModbusRegType())
		{
			case S7COIL:
			{
				...
				printf("Pushing resource %d to Coils.\n", ResourceId);
				break;
			}
			case S7INCONT:
			{
				InputContacts.push_back(ResourceId);
				printf("Pushing resource %d to Input Contacts.\n", ResourceId);
				break;
			}
			case S7HOLDREG:
			{
				...
				printf("Pushing resource %d to Holding Registers.\n", ResourceId);
				break;
			}
			case S7INPUTREG:
			{
				...
				printf("Pushing resource %d to Input Registers.\n", ResourceId);
				break;
			}
			default:
			{
...
int ModbusServer::WaitForConnectRequest(void)
{
	...
	if(server_socket == -1 )
	{
		printf("[ERROR]: Server socket dedication failed%s" , modbus_strerror(errno)) ;
	}
...
ModbusErrorCodes ModbusServer::ReadInputRegs(uint8_t * tcp_query)
{
	...
	if (InputRegs.find(RequestAddress) != InputRegs.end())
	{
		...
		if (result == OK)
		{
			memcpy((DeviceMemory->tab_input_registers + (RequestAddress * 2)), Readdata, itemCount);
		}
		else
		{
			printf("Error reading from PLC dataItems!\n");
